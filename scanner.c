...
{
    memset(&scn->x, 0, sizeof(zbar_scanner_t) + (uintptr_t)scn - (uintptr_t)&scn->x);
                                ...
                                int y)
{
    ...
    volatile int x = scn->x;
    volatile int y0_1 = scn->y0[(x - 1) & 3];
    volatile int y0_0 = y0_1;
    if(x) {
...
void zbar_scanner_get_state (const zbar_scanner_t *scn,
                             ...
                             int *y1_thresh)
{
	zbar_scanner_t *mut_scn = NULL;
    volatile int y0_0 = scn->y0[(scn->x - 1) & 3];
    volatile int y0_1 = scn->y0[(scn->x - 2) & 3];
    volatile int y0_2 = scn->y0[(scn->x - 3) & 3];
    if(x) *x = scn->x - 1;
